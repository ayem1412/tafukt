...
impl PeerWorker {
    ...
    pub async fn run(&mut self, info_hash: [u8; 20], peer_id: [u8; 20], piece_count: usize) {
        if let Err(err) = self.try_run(info_hash, peer_id, piece_count).await {
    ...
    async fn try_run(&mut self, info_hash: [u8; 20], peer_id: [u8; 20], piece_count: usize) -> anyhow::Result<()> {
            ...
            tokio::select! {
                result = self.stream.read_buf(&mut buf) => {
                    ...
                    if let Some(msg) = Message::decode(&mut buf) {
                        self.handle_message(msg, piece_count).await?;
                    }
                }
    ...
    async fn handle_message(&mut self, msg: Message, piece_count: usize) -> anyhow::Result<()> {
        match msg {
            ...
            Message::Bitfield(bits) => {
                ...
                self.peer_bitfield = Bitfield::from_bytes(bits, piece_count);
    ...
    fn maybe_claim_piece(&mut self) {
        ...
        if let Some(index) = piece_manager.claim_piece(&self.peer_bitfield) {
            let piece_len = piece_manager.piece_len(index);
            tracing::debug!("piece len {piece_len}");
