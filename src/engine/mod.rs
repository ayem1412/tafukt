...
impl Engine {
    pub fn new(
        ...
        Self { piece_count, peer_states: HashMap::new(), piece_manager, peer_event_rx, disk_event_rx, block_tx }
    }

    pub async fn run(&mut self, info_dictionary: Arc<InfoDictionary>) {
        loop {
            tokio::select! {
                Some(event_message) = self.peer_event_rx.recv() => {
                    self.handle_peer_event_message(event_message, &info_dictionary).await;
                }
    ...
    async fn handle_peer_event_message(&mut self, event_message: PeerEventMessage, info_dictionary: &InfoDictionary) {
        ...
        match event_message.event {
                ...
                if let Some(peer_state) = self.peer_states.get_mut(&addr) {
                    ...
                    for begin in stale {
                        if let Some(current_piece_idx) = peer_state.current_piece_idx {
                            let length = calculate_block_length(begin, current_piece_idx, info_dictionary);
            ...
            PeerEvent::Unchoke => {
                if let Some(peer_state) = self.peer_states.get_mut(&addr) {
                    ...
                    self.populate_request_pipeline(addr, info_dictionary).await;
                }
            },
            ...
            PeerEvent::Block { piece_index, begin, data } => {
                ...
                let total = info_dictionary.piece_count();
    ...
    async fn populate_request_pipeline(&mut self, addr: SocketAddr, info_dictionary: &InfoDictionary) {
        ...
        if peer_state.current_piece_idx.is_none() && peer_state.remaining_blocks.is_empty() {
            if let Some(piece_idx) = self.piece_manager.claim_piece(&peer_state.bitfield) {
                ...
                peer_state.remaining_blocks = populate_remaining_blocks(info_dictionary.piece_len(piece_idx) as u32);
            } else {
                return;
            }
        }
...
fn calculate_block_length(begin: u32, piece_index: u32, info_dictionary: &InfoDictionary) -> u32 {
    let piece_len = info_dictionary.piece_len(piece_index);
