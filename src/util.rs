...
pub fn generate_peer_id() -> [u8; 20] {
    ...
    let version_bytes = version_to_bytes(APPLICATION_VERSION);
    peer_id[3..7].copy_from_slice(&version_bytes);

    peer_id[7] = b'-';

    (8..20).for_each(|i| peer_id[i] = rng.sample(Alphanumeric) as u8);
...
fn version_to_bytes(version: &str) -> [u8; 4] {
    ...
    [
        parts.first().and_then(|s| s.parse::<u8>().ok()).unwrap_or(0),
